...
if [ "$ME" == "$USERNAME" ] ; then
    ...
    BUILDNUM=$(curl https://api.papermc.io/v2/projects/paper/versions/"$PAPERVERSION" | sed 's/^\[{\(.*\)}]$/\1/' | tr ',' '\n' | tail -n1 | sed s/\]\}//)

    echo ""
    echo "Download paper-$PAPERVERSION-$BUILDNUM"
    wget https://api.papermc.io/v2/projects/paper/versions/"$PAPERVERSION"/builds/"$BUILDNUM"/downloads/paper-"$PAPERVERSION"-"$BUILDNUM".jar -O /opt/mc/server/paper.jar
